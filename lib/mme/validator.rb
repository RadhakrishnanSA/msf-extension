...
module Mme
  module Validator
    TARGET_PATTERN = /\A[a-zA-Z0-9.\-_\/,:]+\z/
    ...
    def self.validate_target!(target)
      ...
      raise ValidationError, "Invalid characters in target: #{target}. Only alphanumeric, dots, hyphens, underscores, slashes, commas, and colons are allowed." unless target.match?(TARGET_PATTERN)
    ...
    def self.validate_report_format!(format)
      ...
      raise ValidationError, "Unknown report format: #{format}. Valid formats: #{VALID_REPORT_FORMATS.join(', ')}" unless VALID_REPORT_FORMATS.include?(fmt)
      fmt
    end
